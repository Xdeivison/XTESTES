...
from config import API_URL, API_KEY, TIME_ZONE
...
def get_data_times(date: str, hour: str) -> pd.DataFrame | None:
    ...
    try:
        response = requests.get(
            f"{API_URL}/estacao/dados/{date}/{hour}/{API_KEY}", timeout=10
        )
...
def get_data_manuals(current_date: str, previous_date: str) -> pd.DataFrame | None:
    ...
    """
    try:
        code_stations = ['83013', '83648', '83557']
        dataframes = []

        for code_station in code_stations:
            url = f"{API_URL}/estacao/{previous_date}/{current_date}/{int(code_station)}/{API_KEY}"
